                ...
                for j, target_word in enumerate(chunk):
                    ...
                    for k, w in enumerate(chunk):
                        ...
                        if k == j:
                            ...
                            display_words.append(f'<font color="#ffff00">{word_text}</font>')
                        else:
...
def burn_subtitles_to_video(video_input, srt_input, facecam_crop, video_output="polished_clip.mp4"):
    """
    ...
    command = [
        ...
        '-i', video_input,                       # Raw source video input path
        '-filter_complex', filter_complex,       # Apply the 9:16 compositing filter graph
        '-map', '[outv]',                        # Map the processed video stream
        '-map', '0:a',                           # Map the original audio stream
        '-c:v', 'libx264', '-crf', '22',         # Use software x264 encoding for reliable video layout
        '-c:a', 'copy',                          # Direct stream-copy audio track
...
def process_video_pipeline(input_video_path, facecam_config="384:216:0:0"):
    ...
    if not os.path.exists(input_video_path):
        print(f"ERROR: Specified source file does not exist: {input_video_path}", flush=True)
        return None
    ...
    try:
        ...
        final_video_path = burn_subtitles_to_video(input_video_path, srt_file_path, facecam_config, video_output=output_path)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print("ERROR: Missing path argument pointing to the source video file.", flush=True)
        sys.exit(1)
